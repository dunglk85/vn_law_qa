...
class TestMemoryRateLimiter:
    ...
    async def test_allows_requests_under_limit(self):
        limiter = MemoryRateLimiter(max_requests=5, window_seconds=60.0)
        ip = "192.168.1.1"
        for _ in range(5):
    ...
    async def test_blocks_requests_over_limit(self):
        limiter = MemoryRateLimiter(max_requests=3, window_seconds=60.0)
        ip = "192.168.1.2"
        for _ in range(3):
            assert await limiter.check(ip) is True
    ...
    async def test_window_slides_after_time_elapses(self):
        limiter = MemoryRateLimiter(max_requests=2, window_seconds=0.05)
        ip = "192.168.1.3"
        assert await limiter.check(ip) is True
        assert await limiter.check(ip) is True
        assert await limiter.check(ip) is False
    ...
    async def test_different_ips_have_independent_counters(self):
        limiter = MemoryRateLimiter(max_requests=1, window_seconds=60.0)
    ...
    async def test_zero_max_requests_blocks_everything(self):
        limiter = MemoryRateLimiter(max_requests=0, window_seconds=60.0)
...
class TestRedisRateLimiter:
    ...
    async def test_allows_under_limit(self):
        with patch("app.adapters.rate_limiters.redis_rate_limiter.aioredis.from_url") as mock_from_url:
            ...
            limiter = RedisRateLimiter(max_requests=5, window_seconds=60.0, redis_url="redis://localhost:6379/0")
